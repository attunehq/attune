...
export async function buildIAMAuthenticationConnectionString(connectionString: string): Promise<string> {
    ...
    if (connectionURL.hostname === "") connectionURL.hostname = process.env["PGHOST"] ?? "localhost"
    if (connectionURL.port === "") connectionURL.port = process.env["PGPORT"] ?? "5432"
    if (connectionURL.username === "") connectionURL.username = process.env["PGUSER"] ?? "postgres"
